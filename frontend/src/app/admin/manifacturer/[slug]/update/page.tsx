...
interface ManifacturerAdminUpdatePageProps {
  params: {
    slug: string;
  };
}
...
const fetchData = async (slug: string) => {
  const response = await axiosInstance.get(`uavs/uav-manifacturers/${slug}/`);
...
const ManifacturerAdminUpdatePage: FC<ManifacturerAdminUpdatePageProps> = ({
  params,
}) => {
  const router = useRouter();
  const { data: manifacturer, isLoading: isLoadingManifacturer } = useQuery(
    "uav-manifacturer-details",
  ...
  const handleSubmit = (e) => {
    ...
    axiosInstance
      .put(
        `uavs/uav-manifacturers/${manifacturer.id}/`,
        {
          translations: {
            en: {
  ...
  return isLoadingManifacturer || isLoadingManifacturers ? null : (
...
export default ManifacturerAdminUpdatePage;
